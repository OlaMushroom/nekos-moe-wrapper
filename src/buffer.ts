 ...
 */
function from_ArrayBuffer_to_Buffer(arrayBuffer: ArrayBuffer): Buffer {
 ...
 */
function from_Buffer_to_ArrayBuffer(buffer: Buffer): ArrayBuffer {
 ...
 */
export function from_File_to_ArrayBuffer(file: File): Promise<any> {
 ...
 */
export function createFile(filePath: string, fileName: string, fileType: "image/jpeg" | "image/png"): File {
  try {
    const imageFile = new File([from_Buffer_to_ArrayBuffer(readFileSync(filePath))], fileName, { type: fileType });
